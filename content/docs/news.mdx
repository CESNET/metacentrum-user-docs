...
<a href="https://docs.cerit-sc.cz/en/docs/news" className="text-fd-primary flex items-center gap-2"><MoveRight />Kubernetes News</a>
<a href="https://metavo.metacentrum.cz/en/news/outages.jsp" className="text-fd-primary flex items-center gap-2"><MoveRight />MetaCentrum outages</a>

## 28/06/2026
### New my.MetaCentrum.cz job monitoring application

We would like to inform about a significant update to our computing job management and monitoring tools. The existing monitoring application, PBSmon, has been replaced by a modern web application, my.MetaCentrum, available at [my.metacentrum.cz](my.metacentrum.cz).
This version brings a clearer user interface reflecting current security and legislative standards.

![pic](/img/meta/computing/mymeta-machines.png)

#### Enhanced Personal Data Protection

In connection with the migration to the new platform and strict adherence to personal data protection regulations (GDPR), the scope of displayed information has been adjusted. For this reason, certain sections and details that you may be familiar with from the previous version of PBSmon are not accessible in the my.MetaCentrum application. 
[See Documentation](../docs/computing/my-metacentrum) 



## 2/06/2026
### New version of Alphafind

CERIT-SC has launched the second generation of [AlphaFind](https://alphafind.ics.muni.cz)! This fast and reliable tool for discovering similar protein structures combines massive datasets with advanced machine learning and high-performance computing power. The service is fully and freely available to the wider scientific community.
![AlpaFind logo](/img/meta/software/alphafind_logo.png)

AlphaFind v2 is a search engine for protein similarity in the complete (214-million) AlphaFold (v4) and TED (365-million) databases. It allows search at the protein-chain level, with residue-level pLDDT masking at fixed thresholds (70, 80, 90) to limit comparison to stable regions, and enables searches across the database of TED domains in a single-domain or multi-domain search mode that combines all domain matches within a single score.

The search works by fast approximate similarity search on vector embeddings of protein chains and domains followed by the search refinement using TM-Score on raw CIF files.

Results can be downloaded in CSV format, shared via a permanent link, or used to launch a new analysis directly from the results table. This facilitates seamless integration into subsequent research pipelines.
...
* When to use **skirit**? Use it for more demanding interactive work and operations requiring higher computing power on the frontend itself.

**Important migration information**

**SSH key changes**: In connection with the renaming and hardware replacement, your SSH client may warn you about a change in fingerprints (host keys). This is expected behavior in this case. You can verify the list of [SSH keys in the documentation](../docs/access/security/connect-auth)

## 23/04/2026
### PBS cheatsheet for MetaCentrum users
A new, succinct PBS cheat sheet is out! Check out the [Tutorials](./tutorials#general) or download it directly from [this link](/img/meta/tutorials/PBS_cheatsheet.pdf).
![PBS pdf](/img/meta/tutorials/PBS_cheatsheet_view.png)

## 31/03/2026
### New Clusters in MetaCentrum: Infrastructure Expansion with CESNET and CERIT-SC Resources

New computing clusters owned by CESNET and CERIT-SC have been integrated into the MetaCentrum infrastructure. This hardware update increases CPU and GPU capacity.

1. Owner: CESNET (hildor[1-20].metacentrum.cz, deimos[1-13].meta.zcu.cz, haldan[1-15].metacentrum.cz), České Budějovice, Plzeň, Brno (48 nodes / 6 144 CPU cores)

2. Owner: CERIT-SC (grogu[1-3].cerit-sc.cz, grogu[4-8].cerit-sc.cz), Brno (8 nodes / 768 CPU cores / 12 GPUs)

Link: [MetaCentrum Hardware List](https://my.metacentrum.cz/machines)
...
**New Skirit-Lite for Lightweight Tasks**
...
## 09/02/2026
### Encryption for Interactive Jobs
Starting this week, we are enabling encryption for interactive jobs. This brings two important changes:
* **Kerberos ticket renew**: If you do not have a valid ticket when connecting to a job, you will be prompted to renew it.
* **3-hour limit**: You must connect to your interactive job within 3 hours of its start, or it will be automatically cancelled.
...
A complete list of available computing servers can be found here: [https://my.metacentrum.cz/machines](https://my.metacentrum.cz/machines)
