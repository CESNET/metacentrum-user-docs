...
---
```bash
Matlab at OnDemand   # MetaCentrum OnDemand web app
Matlab at Kubernetes # Kubernetes web app
module avail matlab/ # as a module
```
...
There are tree ways how to use Matlab in an graphical interface

### Option1: Running Matlab in OnDemand

The most straightforward way to run Matlab is through the [OnDemand service](https://ondemand.metacentrum.cz), which allows users to access Matlab directly from their web browser. This option provides an intuitive and user-friendly interface, making it an excellent choice for most users.

Choose `Matlab` button among the `applications`. 

### Option2: Running Matlab via Jupyter Hub in Kubernetes

   * Ease of Use: JupyterHub offers an easy way to launch Matlab directly from your Jupyter Notebook environment.
   ...
   * Supported Version: More Matlab versions are supported through this option.

To learn more, see [Jupyter documentation](https://docs.cerit.io/en/docs/web-apps/jupyterhub) where you can find other necessary information.


### Option 3: Running Matlab in Remote desktop using module
...
<Callout  type="warn" title="Warning">
PBS scheduler tracks only base licenses for older Matlab versions, not toolboxes. Also, **when you submit the job with old Matlab, use the `-l matlab50=XX` option to request base license,  do not use `-l matlab=XX`**.
</Callout>
