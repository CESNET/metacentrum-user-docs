...
MetaCentrum does not own a licence for all users. According to the VASP licence agreement, we can make VASP available to every research group or institution, which proves the licence ownership. 

<Callout type="info"> In order to access the VASP software installed in MetaCenter, users are required to register all group users through the official 
VASP License Management Portal https://www.vasp.at/sign_in/portal/. 
</Callout>

Please send us an email address that is registered by the VASP developers, and we will take the necessary steps to make the application available to you in the corresponding version. 
Vasp-utils is available without registration, but its use should be cited.
