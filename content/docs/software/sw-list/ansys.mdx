...
---
```bash
Ansys at OnDemand   # MetaCentrum OnDemand web app
Ansys at Kubernetes # Kubernetes web app
module avail ansys/
```
...
Parallel (single node) computation on N processors:

```
module add ansys/2025-R2
#### Run the computation, e.g.
lsdyna ncpu=$PBS_NCPUS i=myinput.k
```
...
```bash
skirit$ N=4
skirit$ qsub -l select=1:ompthreads=${N}:mem=Zgb -l ansys-dyna=1  -l ansys-dysmp=$((N-1)) batch_script.sh
```
