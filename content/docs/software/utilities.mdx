...
- usage:
  ...
  ```
  Run the following command to view files consuming your filesystem quota:
  ```bash
  $ check-local-quota
  ```
  Inspect and Manage Files: If the listed files contain valuable data, copy them to your home directory under `/storage` for safekeeping.  
  Once copied, remove the unnecessary files or directories to free up space:
  ```bash
  $ rm file_XX
  $ rm -rf directory_XX
  ```
  - see more on [FAQ/No space left on a node](../support/faqs-content/no-space-left)
