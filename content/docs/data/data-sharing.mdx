...
<Callout type="warning" title="Project directories reside in S3 storage">
Starting by Q3/2025, the data in project directories were transferred to [object storage (S3) technology](https://docs.du.cesnet.cz/en/docs/object-storage-s3/s3-service), which is different from a filesystem used in MetaCentrum storages. This can lead to some caveats, see [](take care).
</Callout>
...
Due to different style of data storage, the project directories are not fully compatible with Unix file permissions.
