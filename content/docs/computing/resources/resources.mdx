...
The basic cxommand for runninng a job is
...
<Callout type="warn" title="There is no default scratch">
    As opposed to **ncpus**, **mem** and **walltime**, there is no default size for **scratch directory**. A scratch directory is always created with the start of the job, however if you need to guarantee a certain space for data, we strongly recommend to specify type and size of a scratch dir.
</Callout>
...
For job with large memory usage (>500 GB of RAM), you must use a specific queue. Add `-q large_mem@pbs-m1.metacentrum.cz` to your `qsub` - see [large_mem queue](./queues#specific-queues).
...
{/*
Chunk vs job - odsunout na konec / do advanced sekce 
## Chunk vs job
