...
Simple configuration of `/etc/idmapd.conf` can look the following way: `Domain = META`. Users from domain @META will be mapped through to files `/etc/passwd` and `/etc/group`. This means that for the identity `user_123@META` there must exist a record in `/etc/passwd` with name `user\_123`. Ordinary tools (`ls -l`) will show the names properly if the name in given files exists for their principal. For nonexisting name user will be mapped as nobody and nogroup.
