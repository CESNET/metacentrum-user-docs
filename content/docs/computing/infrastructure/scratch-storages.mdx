...
| Scratch type | PBS resource | Speed | Do data survive end of job? | Path |
|--------------|--------------|-------|-----------------------------|------|
| [Local](#local-scratch) | `scratch_local` | normal | no | $SCRATCHDIR |
| [SSD](#fast-ssd-scratch) | `scratch_ssd` | fast | no | $SCRATCHDIR |
| [in RAM](#scratch-in-ram) | `scratch_shm` | ultra fast | no | $SCRATCHDIR |
| [Shared](#shared-scratch) | `scratch_shared` | fast | no | $SCRATCHDIR |
| [Shared scratch on Bee cluster](#shared-scratch-on-cluster-bee) |  `scratch_shared` AND `cl_bee=True` | fast | yes | data was moved `/scratch/multijob-brno` |
| [Multijob fast](#multijob-scratch-fast) | `multijob` | fast | yes | manual, `/scratch/multijob-[brno/plzen]` |
| [Multijob global](#multijob-scratch-global) | `multijob` | slow | yes | manual, `/scratch/multijob-global` |
...
<Callout type="info" title="Feature in preparation">
    This feature is currently in beta now. Currently available from skirit, skirit-lite, nympha and perian frontends.
</Callout>
...
The in-between-jobs consistency of path to scratchdir must be kept manually by the user.

```bash
# Run 1st job

qsub -I -l walltime=01:00:00 -l select=1:multijob=brno

(TRIXIE)user123@bee4:~$ cd /scratch/multijob-brno/user_123/
(TRIXIE)user123@bee4:~$ mkdir my_new_multijob_project
(TRIXIE)user123@bee4:~$ SCRATCHDIR=/scratch/multijob-brno/user_123/my_new_multijob_project/
# cp whatever data to SCRATCHDIR
# run the job no.1
(TRIXIE)user123@bee4:~$ exit

# Run another job on the same data

qsub -I -l walltime=01:00:00 -l select=1:multijob=brno # MUST be brno this time

(TRIXIE)user123@bee4:~$ SCRATCHDIR=/scratch/multijob-brno/user_123/my_new_multijob_project/
(TRIXIE)user123@bee4:~$ cd $SCRATCHDIR
# run the job no.2
(TRIXIE)user123@bee4:~$ exit
```
...
<Callout type="info" title="Will be renamed soon">
    This scratch type has been discontinued by end of September 2026 in favour of multijob scratches. We keep this section for backwards reference only while the multijob scratches are in beta run.
</Callout>
