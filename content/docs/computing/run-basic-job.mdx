...
Once your account is activated, you can log in to the MetaCentrum infrastructure and start running jobs.
...
We are using the `tarkil.metacentrum.cz` login server (frontend), however, we recommend choosing the one closest to your physical location to minimize the network latency – you can inspect the [List of Frontends](../computing/infrastructure/frontends).
...
For detailed login instructions (including Windows/PuTTY), see the [Log in guide](../access/log-in).
...
After a short while these files will be created in the directory from where the job was submitted:

- `job_info.<jobID>.<server_name>` (output file with job info your batch script has explicitly created)
- `out_count.txt` (output file created by the Python script with the number of lines in the input file written to it)
- `my_counter.o<jobID>` (default file with standard output --`STDOUT`)
- `my_counter.e<jobID>` (default file with standard error -- `STDERR`; empty unless the job logged errors)
